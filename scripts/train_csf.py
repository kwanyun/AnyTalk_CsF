...
"""
Character-specific Fine-tuning (CsF) for Hallo.

CsF adapts a pre-trained Hallo network to rendered images of one target
character. During training the audio embedding is zeroed, all pre-trained
components are frozen, and only the spatial residual blocks in the denoising
UNet are optimized. This preserves Hallo's audio and temporal motion priors.
"""
...
def train_csf_process(cfg: argparse.Namespace) -> None:
    """
    Fine-tune the spatial residual blocks using the given CsF configuration.
    ...
    # Freeze everything before enabling only the spatial residual blocks.
    ...
    net = Net(reference_unet,denoising_unet,face_locator,reference_control_writer,reference_control_reader,imageproj,audioproj).to(dtype=weight_dtype)
    ...
    assert not missing_keys and not unexpected_keys, \
        "Failed to load the complete pre-trained Hallo checkpoint."

    if accelerator.is_main_process:
        logger.info("Loaded weights from %s", cfg.pretrained_model_path)
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                model_pred = net(
                    ...
                    audio_emb=torch.zeros_like(
                        batch["audio_tensor"]).to(
                        dtype=weight_dtype),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./configs/train/csf.yaml"
    )
    args = parser.parse_args()

    config = load_config(args.config)
    train_csf_process(config)
