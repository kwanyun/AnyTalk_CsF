...
Example:
python scripts/inference.py --source_image character.png \
    --driving_audio speech.wav --net_dir outputs/csf/character/modules/net-100.pth \
    --output outputs/inference/character.mp4
"""
...
def inference_process(args: argparse.Namespace):
    ...
    output_parent = os.path.dirname(os.path.abspath(config.output))
    ...
    print("loaded weight from ", net_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--driving_audio", type=str, required=True,
                        help="driving audio")
    parser.add_argument(
        "--output", type=str, help="output video file name",
        default="outputs/inference/output.mp4")
    parser.add_argument(
        "--pose_weight", type=float, default=0.0,
        help="weight of pose", required=False)
    parser.add_argument(
        "--face_weight", type=float, default=1.0,
        help="weight of expression", required=False)
    parser.add_argument(
        "--lip_weight", type=float, default=2.0,
        help="weight of lip motion", required=False)
    parser.add_argument(
        "--face_expand_ratio", type=float, help="face region", required=False)
    parser.add_argument(
        "--net_dir", "--checkpoint", type=str,
        help="path to a CsF net-*.pth checkpoint", required=False)
